...
import { customersMapper } from "../mappers/customer.mapper";
...
export const customersSortValueToFieldValue = (sortValue: ECustomersSortVariant) => {
  switch (sortValue) {
    ...
    case ECustomersSortVariant.id_desc:
      return { id: "desc" };
    case ECustomersSortVariant.fullName_asc:
      return { fullName: "asc" };
    case ECustomersSortVariant.fullName_desc:
...
export async function adminCustomersLoader({ request }: LoaderFunctionArgs) {
  ...
  return json({ customers: customers.map(customersMapper), query: data, pagination });
}
